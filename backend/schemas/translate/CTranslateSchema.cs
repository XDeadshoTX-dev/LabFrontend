...
{
    class CTranslateSchema : AbstractTranslateSchema
    {
        public CTranslateSchema(int deep, AbstractBlock block)
        {
        ...
        public override string InsertCode(Match match)
        {
            ...
            string newContent = string.IsNullOrEmpty(content)
                ...
                : $"\n{this.block.GetIndent(this.deepSchema + 1)}{content}\n{this.block.GetIndentCode(this.deepSchema + 1)}\n";

            return $"{this.block.GetIndent(this.deepSchema)}" +
                   $"{openingBrace}" +
                   $"{newContent}" +
